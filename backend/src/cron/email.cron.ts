...
export class EmailCronService {
  ...
  async processEmailQueue() {
    ...
    if (this.isProcessing) {
      this.logger.warn(
        '[Email Worker] Previous batch is still processing. Skipping this cycle.',
      );
      return;
    }
    ...
    try {
      ...
      this.logger.log(
        `[Email Worker] Processing ${emailsToSend.length} emails...`,
      );
      ...
      for (const email of emailsToSend) {
        try {
          ...
          this.logger.log(
            `[Email Worker] ✅ Successfully sent to: ${email.recipientEmail}`,
          );
        } catch (sendError) {
          email.status = 'Failed';
          email.retryCount += 1;
      ...
      if (
        dbError instanceof QueryFailedError &&
        typeof dbError.driverError === 'object' &&
        dbError.driverError &&
        (dbError.driverError as { code?: string }).code === '42P01'
      ) {
        if (!this.missingTableWarningLogged) {
          this.logger.warn(
            '[Email Worker] emails_queue table not found, skipping cron until schema is ready.',
          );
      ...
      this.logger.error(`[Email Worker] Database connection error: ${message}`);
    } finally {
