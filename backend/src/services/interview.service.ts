...
import { DataSource, In, LessThanOrEqual, MoreThanOrEqual, Repository, EntityManager } from 'typeorm';
...
import { InterviewStatus } from 'src/enum/interview-status.enum';
...
export class InterviewService {
    ...
    private async checkAndLockAvailability(
        ...
        if (excludeInterviewId) {
            ...
            if (oldPanelIds.length > 0) {
                ...
                await manager.update(InterviewerAvailability, {
    ...
    async create(data: InterviewCreateDto, userId: number) {
        ...
        try {
            ...
            if (!application || ['Selected', 'Rejected'].includes(application.status)) {
                throw new NotFoundException('Application not found or no longer eligible for interview.');
            }

            // 4. Extract Panel IDs and Lock Availability
            const panelIds = data.panel.map(p => p.employeeId);
            await this.checkAndLockAvailability(queryRunner.manager, panelIds, start, end);
        ...
        `,
            ...
            if (err && !(err instanceof ConflictException) && !(err instanceof BadRequestException)) {
                ...
                console.error('Transaction failed. Generating rollback for potential orphaned Google Meet event.');
            }
    ...
    async reschedule(
        ...
        try {
            ...
            await this.checkAndLockAvailability(queryRunner.manager, panelIds, start, end, id);
            ...
            if (status === InterviewStatus.CANCELLED) {
                ...
                if (interview.googleCalendarEventId) {
                    try {
                        ...
                        console.error('[Google Meet] Failed to delete event, continuing DB update:', error);
                    }
                }
                ...
                if (panelIds.length > 0) {
                    ...
                    await queryRunner.manager.update(InterviewerAvailability, {
