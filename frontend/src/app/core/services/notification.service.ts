...
export class NotificationService {
  ...
  public unreadCount = computed(() => this._notifications().filter(n => !n.isRead).length);
  ...
  constructor(private http: HttpClient, private authService: AuthService) {
    ...
    effect(() => {
      ...
      if (currentUser?.id) {
        this.socket.on(`notification_${currentUser.id}`, (notif: InAppNotification) => {
  ...
  markRead(id: string): Observable<ApiResponse<InAppNotification>> {
    ...
    return this.http.patch<ApiResponse<InAppNotification>>(`${this.base}/${id}/read`, {}).pipe(
      tap(() => {
