...
export class EmailQueueService {
    ...
    async handleApplicationSubmitted(payload: any) {
        this.logger.log(`[Queue] Received application from ${payload.candidateName}. Saving email to Database...`);
        ...
        `,
            ...
            this.logger.log(`[Queue] ✅ Saved thank you email for ${payload.candidateEmail} to the queue.`);
        } catch (error: any) {
            this.logger.error(`[Queue] ❌ Error saving email to DB: ${error.message}`);
        }
    }
}