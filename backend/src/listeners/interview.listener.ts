...
export class InterviewListener {
    ...
    async handleInterviewScheduledEvent(payload: any) {
        try {
            ...
            this.logger.log(`Successfully processed scheduled event for Application ID: ${payload.applicationId}`);
        } catch (error) {
            this.logger.error(`Error processing interview.scheduled event: ${error.message}`);
        }
    ...
    async handleInterviewRescheduled(payload: any) {
        try {
            ...
            this.logger.error(`Error processing interview.rescheduled event: ${error.message}`);
        }
    }
    ...
    async handleInterviewCancelled(payload: any) {
        try {
            ...
            this.logger.error(`Error processing interview.cancelled event: ${error.message}`);
        }
    }
}